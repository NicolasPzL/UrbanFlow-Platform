...
import React from "react"; // Imported for React.MouseEvent type
...
  const tools: Tool[] = user?.rol === 'cliente' ? [
    {
      title: "Mi Dashboard",
...
          {systemStats.map(({ icon: Icon, ...stat }, index) => (
...
            <Button 
              size="lg"
              onClick={() => onViewChange(user?.rol === 'cliente' ? 'geoportal-public' : 'geoportal-detail')}
              className="px-8"
            >
